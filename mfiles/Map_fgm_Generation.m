%% This function generates NIfTI volumes of all spectra to keep fGM values. 
% Inputs: P info struct, All concentration values parsed from LCModel output files.
% All maps will be saved under ~nifti/fGM_maps folder as a nifti file.
% Author: Sevim Cengiz,  Bogazici University , 2022 - For revison.
% Contact: sevim_cengiz@icloud.com

function Map_fgm_Generation(Pinfo,fgm,coreg_path,Metabolites)
...
% MRIFOVMask_file=load_untouch_nii([coreg_path,filesep,Pinfo.sparname,'_',Metabolites(n).name,'_FOV_mask.nii']);
 ...
 %% LCModel concentration results are used after exclusion criteria.
            ...
            for icol=1:col
                ...
                if includedvoxels(1,isli,irow,icol)==1 || includedvoxels(1,isli,irow,icol)==0
                    ...
                    for kk=1:xx
                        ...
                        ly=locations(1,2);
                        lz=locations(1,3);
                        MRIFOVMask_file.img(lx,ly,lz)=fgm(1,isli,irow,icol);
                    end
    ...
    out_mask=[Pinfo.spectrapath,filesep,'nifti',filesep,'fgm_maps'];
    if ~exist(out_mask,'dir')
        mkdir(out_mask);
    end
    
   
     Mask_path=[out_mask,filesep,Pinfo.sparname,'_',Metabolites(n).name,'_fgm.nii'];
