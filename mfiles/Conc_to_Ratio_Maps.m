...
function Conc_to_Ratio_Maps(Pinfo)

out_MetMaps=[Pinfo.spectrapath,filesep,'nifti',filesep,'conc_maps',filesep];
...
CrPCr=dir([out_MetMaps,Pinfo.sparname,'*_Cr+PCr_conc.nii']);
CrPCr_path=[CrPCr.folder,filesep,CrPCr.name];
CrPCr_mask=load_untouch_nii(CrPCr_path);
[xsz ysz zsz]=size(CrPCr_mask.img);

out_MetMaps_CrPCrRatio=[Pinfo.spectrapath,filesep,'nifti',filesep,'conc_maps_to_tCr_ratio',filesep];
...
for n=1:metno(1,2)
    ...
    Metab_path=[out_MetMaps,Pinfo.sparname,'_',Metabolites(n).name,'_conc.nii'];
    ...
    MetabCrRatio_path=[out_MetMaps_CrPCrRatio,filesep,Pinfo.sparname,'_',Metabolites(n).name,'_conc_to_tCr_ratio.nii'];
...
Ins=dir([out_MetMaps,Pinfo.sparname,'*_Ins_conc.nii']);
Ins_path=[Ins.folder,filesep,Ins.name];
Ins_mask=load_untouch_nii(Ins_path);

out_MetMaps_InsRatio=[Pinfo.spectrapath,filesep,'nifti',filesep,'conc_maps_to_Ins_ratio',filesep];
...
for n=1:metno2(1,2)
    ...
    Metab_path=[out_MetMaps,Pinfo.sparname,'_',Metabolites2(n).name,'_conc.nii'];
    Metab_mask=load_untouch_nii(Metab_path);
    ...
    MetabInsRatio_path=[out_MetMaps_InsRatio,filesep,Pinfo.sparname,'_',Metabolites2(n).name,'_conc_to_Ins_ratio.nii'];
    Metab_mask.untouch=1;
